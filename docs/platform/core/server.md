...
```
The server world is made up of the wasip2 imports and the `wasi:http/incoming-handler@0.2.0` export for http, but also includes the `hayride:http/config@0.0.61` export, which exposes a config `get` to enable the Hayride to get server configuration such as the address to serve.
...
- `hayride:core/version@0.0.61`: A Hayride core interface which provides a function to check the latest version of Hayride releases.
...
- `hayride:http/config@0.0.61`: A hayride specific config interface to allow server implementations to expose their own server configuration through a `get` function. See below for an example of what this wit definition looks like.
